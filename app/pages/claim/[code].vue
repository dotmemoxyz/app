<template>
  <div>
    ...
    <div class="mx-auto flex max-w-xl flex-col items-center gap-[32px] p-4 md:mt-24">
      ...
      <div
        v-if="hasSocials"
        class="flex w-full flex-wrap items-center justify-center gap-[12px] rounded-[12px] p-[16px]"
      >
        <!-- Telegram -->
        <a
          v-if="data?.customize?.telegram"
          :href="getTelegramUrl(data.customize.telegram)"
          target="_blank"
        ...
        <a
          ...
          :href="getInstagramUrl(data.customize.instagram)"
          target="_blank"
          class="flex items-center gap-[6px] px-[14px]"
          rel="noopener noreferrer"
        >
