...
export default defineEventHandler(async (event) => {
  ...
  const [rawData, err] = await $fetch<MemoDetail>(`${RUNTIME_CONFIG.apiUrl}/manage/memos/${chain}/${id}`, {
    method: "GET",
    headers: {
  ...
  if (err) {
    ...
    throw createError({
      statusCode: 500,
      message: `[API::COLLECTED] Failed to fetch memo details: ${err.message || "Unknown error"}`,
    });
  }
